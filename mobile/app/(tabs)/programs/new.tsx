...
import { apiErrorMessage, displayToLbs, weightShort, type Exercise } from '@sebu/shared'
...
// Weights live in DISPLAY units in the form (reps/weight per set, same shape the shared
// ExerciseFormCard edits); converted to lbs + mapped to target_reps/target_weight only in
// the submit payload — 1:1 with web AddProgram.
...
const KEYPAD_DONE_ID = 'program-new-keypad-done'
...
export default function AddProgram() {
  ...
  // Stable + immutable handlers (see workouts/new.tsx) so the memoized ExerciseFormCard
  // only re-renders the edited card.
    ...
    <Screen>
      ...
      >
        <View className="gap-6 py-4">
          ...
          <View>
            <FieldHeader icon={BookOpen} label="Program Name" hint="(required)" />
            <Field
              value={formData.name}
      ...
      <View className="-mx-5 flex-row gap-3 border-t border-surface-border bg-surface-base px-5 pb-2 pt-3">
        ...
        <Button title="Save Program" className="flex-1" onPress={handleSubmit} loading={loading} />
      </View>
